...
#define JT_WRITEDBG	0x16
#define JT_READDBG	0x17
...
typedef struct
{
	...
	reentrant unsigned long (*	readcpsr)(void);
	reentrant unsigned char	(*	writecpsr)(unsigned long value);
	reentrant unsigned char	(*	stop)(void);

	/*reentrant unsigned char (*set_break)(unsigned long address, unsigned char soft);
	reentrant unsigned char (*clear_break)(unsigned long address);*/
	reentrant  void (*write_dbgreg)(unsigned char reg, unsigned long address);
	reentrant unsigned long (*read_dbgreg)(unsigned char address);
	
	reentrant unsigned char (*go)(void);
	reentrant unsigned char (*isstopped)(void);
...
extern const dev_int	arm7_interface;				// arm labs jtag interface
extern unsigned char connected;				// true when device connected
extern unsigned char error_code;
extern void arm_save_dbgsts(void);
extern void arm_restore_dbgsts(void);
