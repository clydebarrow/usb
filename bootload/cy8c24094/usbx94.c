...
unsigned char xdata	USB_status[NUM_ENDPOINTS] @ 0x3F7;
...
USB_control(void)
{
	...
	if(USB_status[0] & RX_READY) {
		...
		//printf("stp %X/%X/%X/%X", EP0_DR[REQ_REQ], EP0_DR[REQ_VALUE+1], EP0_DR[REQ_VALUE], cnt);
		//putch(EP0_DR[REQ_REQ]+0x30);
