...
main(int argc, char ** argv)
{
	...
	VEC_ITERATE(devs, cp, char *) {
		if(strstr(cp, "usbstor#disk&ven_hi-tech&prod_zipcard")) {
			...
			message("Sending MAINT command to %s", cp);
			hFile = CreateFile(cp,
			...
			fp = fopen(outname, "wb");
			if(!fp)
				error("Couldn't create output file %s", outname);
			fwrite(((sptwb+1)), 1, LOGSIZE, fp);
			fclose(fp);
			message("Wrote log file to %s", outname);
			exit(0);
		}
	}
