...
static USB_CONST unsigned char SERIAL_NUMBER[26] =
{
...
USB_isr(void) @ USB0
{
	...
	csr = USB_read(CMINT);
	if(csr & SUSINT)			// suspend signalling detected
		USB_status[0] |= USB_SUSPEND;
	channels = USB_read(IN1INT) | USB_read(OUT1INT);
	if(channels & 1) {	// Endpoint 0
		csr = USB_index_read(0, E0CSR);
		if(csr & STSTL0)			// stall has been sent - clear the bit
			USB_write(E0CSR, 0);	// clear E0CSR - addr already set
