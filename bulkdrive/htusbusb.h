...
#define BULKUSB_REGISTRY_PARAMETERS_PATH  \
	L"\\REGISTRY\\Machine\\System\\CurrentControlSet\\SERVICES\\HI-TECH USB\\Parameters"


typedef struct _BULKUSB_PIPE_CONTEXT {

    BOOLEAN PipeOpen;		// true when pipe is open
} BULKUSB_PIPE_CONTEXT, *PBULKUSB_PIPE_CONTEXT;

typedef struct _BULKUSB_PIPEINFO {
	ULONG	Pipeno;			// pipe number
